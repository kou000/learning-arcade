# learning-arcade リポジトリ構成・アーキテクチャ

このドキュメントは、`learning-arcade` 全体の構成と責務分割を AI/開発者向けに整理したものです。  
`docs/` 配下のため、通常ビルドには含まれません。

## 1. 技術スタック

- フロントエンド: React 18 + TypeScript
- ビルド: Vite 5
- スタイリング: Tailwind CSS（`src/styles/index.css` で共通CSSも定義）
- ルーティング: 外部ライブラリなし（`window.location.hash` を手動パース）
- 状態保存: `localStorage`（一部機能）
- PWA: `public/manifest.webmanifest` + `public/sw.js`（Service Workerでオフライン再訪問を補助）

## 2. ディレクトリ構成

```text
src/
  App.tsx                # ルーティングと画面切替の中心
  main.tsx               # Reactエントリポイント（Service Worker登録含む）
  assets/                # 画像アセット
  styles/
    index.css            # Tailwind読込 + 共通アニメーション/変数
  domain/
    specs/               # 検定仕様（級別ルール・型）
    generator/           # 問題生成ロジック
  features/
    arcade/              # アーケードTOP
    english/             # 英語練習機能（アルファベット/単語なぞり・単語ゲーム）
    practice/            # テスト/れんしゅうモード（views/, components/, hooks）
    piano/               # ピアノ練習機能（譜面データ/音声エンジン含む）
    soroban/             # ゲームモード（views/, components/ + 保存状態）
  ui/
    components/          # 汎用UI部品（Button, Selectなど）
public/                  # PWA配信ファイル（manifest, service worker, icon）
docs/                    # 実装向けドキュメント（ビルド非対象）
```

## 3. レイヤー設計

依存方向は次を基本にする。

1. `domain`（純ロジック）
2. `features`（画面/ユースケース）
3. `ui`（再利用可能な表示部品）
4. `App.tsx`（画面統合・遷移）

`domain` は React に依存しない。  
`features` は `domain` と `ui` を利用して画面要件を実装する。

## 4. ルーティング設計

`src/App.tsx` で `hashchange` を監視し、`Route` を切り替える方式。

- `/#` : `ArcadeHome`
- `/#/soroban` : `PracticePage`
- `/#/soroban/register` : `RegisterTopPage`
- `/#/soroban/register/play` : `RegisterGamePage`
- `/#/soroban/shop` : `ShopPage`
- `/#/soroban/shop/payment/:itemId` : `ShopPaymentPage`
- `/#/soroban/shelf` : `ShelfPage`
- `/#/soroban/gacha` : `GachaPage`
- `/#/soroban/cards` : `CardBookPage`
- `/#/soroban/stickers` : `StickerBookPage`
- `/#/soroban/log` : `ProblemLogPage`
- `/#/soroban/badges` : `SnackBadgeBookPage`
- `/#/soroban/snack/top` : `SnackBudgetTopPage`
- `/#/soroban/snack` : `SnackBudgetGamePage`
- `/#/soroban/snack/result` : `SnackBudgetResultPage`
- `/#/soroban/admin` : `RegisterAdminPage`
- `/#/piano/practice` : `PianoPracticePage`
- `/#/english` : `EnglishHomePage`
- `/#/english/alphabet` : `AlphabetPracticePage`
- `/#/english/words` : `WordPracticePage`
- `/#/english/word-game` : `WordGamePage`

補足:

- `VITE_REGISTER_ADMIN_MODE` が truthy（`1/true/on`）ならグローバルに `ADMIN MODE` バッジ表示。
- `/#/soroban/admin` では adminモードを画面操作で ON/OFF でき、必要に応じて環境変数判定へ戻せる（セッション単位）。

## 5. ドメイン層（問題生成）

主要ファイル:

- `src/domain/specs/types.ts` : 仕様型（`Grade`, `Subject`, `ExamBody` など）
- `src/domain/specs/kenteiSpec.ts` : 級別出題仕様データ
- `src/domain/generator/index.ts` : `generateProblems` の入口
- `src/domain/generator/*.ts` : 種目別生成（`mul`, `div`, `mitori`, `denpyo`）

出題の中心API:

- `generateProblems(grade, subject, examBody): Problem[]`
- `subjectMinutes(grade, subject, examBody): number`

`Problem` は `question`, `answer`, `kind` を持つ共通型。
全珠学連では 1〜8級の暗算種目（`mentalMul`, `mentalDiv`, `mentalMitori`）も同じ `generateProblems` 経由で生成する。
問題用紙の点数表示は、150点満点と問題数から算出する。

## 6. 機能層（features）

## 6.1 arcade

- `ArcadeHome.tsx`
- 学習ゲームの母艦トップ画面。
- TOP右下に「キャッシュ更新」と「admin」ボタンを表示し、`admin` 押下時はパスワード認証後に `#/soroban/admin` へ遷移。

## 6.2 piano（ピアノ練習）

- `views/PianoPracticePage.tsx`
- 子ども向けの iPad 横画面を想定した練習画面を提供。
- 曲データは `domain/score.ts` で `tempoBpm + Section -> Measure -> NoteEvent` 構造として管理。
- 音声再生は `audio/audioEngine.ts` に集約し、鍵盤ハイライト・録音・テンポ切替・お手本再生を提供。

## 6.3 english（英語練習）

- `views/EnglishHomePage.tsx`
- `views/AlphabetPracticePage.tsx`
- `views/WordPracticePage.tsx`
- `views/WordGamePage.tsx`
- アルファベットをなぞって練習する画面。
- 英語トップから「れんしゅう」「ゲーム」を選ぶ。
- 単語を手本の上になぞって練習する画面。
- 単語ゲームはレジゲーム風に、吹き出しの注文を見て単語をなぞり、既存のコインへ報酬を加算する。
- 対象は大文字 `A`〜`Z` と小文字 `a`〜`z` の1文字練習、および小文字の短い身近な英単語。
- ガイド点列は `domain/letterGuides.ts` に定義し、判定しきい値は画面側で固定値を使う。
- DTW、開始点、終了点、長さ、方向、カバー率の判定は `domain/traceJudge.ts` に集約する。
- 単語練習/単語ゲームは `domain/wordPracticeItems.ts` で定義した身近な短い単語を扱い、既存の小文字ガイドを縮小・配置して単語ガイドを構成する。
- 単語ゲームの保存は既存キー `learning-arcade:soroban-state` 内の `englishWordProgress` / `englishWordPlayConfig` を使う。
- 将来の単語練習に備え、画面は `domain/practiceItems.ts` の `PracticeItem` を扱う。
- 結果は保存しない。

## 6.4 practice（テスト/れんしゅう）

- `views/PracticePage.tsx` が親。
- テストモード:
  - `ProblemSheet`, `AnswerSheet`, `TimerBar`, `useTimer`
- れんしゅうモード:
  - `OneByOnePractice`

保存連携:

- `loadPracticeConfig` / `savePracticeConfig`（`features/soroban/state.ts`）

## 6.5 soroban（ゲームモード）

- `RegisterTopPage` : ゲームTOP（条件選択）
- `RegisterGamePage` : レジゲーム本編
- `ShopPage` : 報酬ショップTOP（商品一覧）
- `ShopPaymentPage` : 支払い画面（商品ごと）
- `ShelfPage` : 複数の棚スロット配置（棚を切り替え、タップ起点モーダルで購入済みグッズを配置）
- `GachaPage` : けいまるカード/シールのガチャガチャ（カードは未所持確定、シールは重複あり）
- `CardBookPage` : けいまるカードの所持一覧
- `StickerBookPage` : シール帳（4ページ構成、所持シールを自由配置）
- `ProblemLogPage` : レジゲームの問題単位ログから、選択日の集計と月曜始まりの週別日次集計を表示
- `SnackBudgetGamePage` : 300円おやつゲーム（棚の商品をドラッグ＆ドロップでカゴに入れ、お会計時に結果判定）
- `SnackBadgeBookPage` : ゲーム内バッジ図鑑（300円おやつゲーム + レジゲームのバッジを表示）
- `RegisterAdminPage` : セーブデータ編集（管理者画面）
- `SceneFrame` : ゲーム系共通フレーム
- `catalog.ts` : ショップ商品定義
- `cardCatalog.ts` : カード定義（ショップ商品とは別管理）
- `shelfCatalog.ts` : 棚ID/解放条件の定義
- `state.ts` : ゲーム進行と保存管理

重要ルール:

- ゲームの出題値は `generateProblems` を再利用（練習と同一ロジック）。
- 見せ方のみゲーム向けに変換。

## 7. 状態管理・永続化

単一キーで保存:

- key: `learning-arcade:soroban-state`
- データ:
  - `practiceConfig`
  - `registerProgress`
  - `registerPlayConfig`
  - `englishWordProgress`
  - `englishWordPlayConfig`
  - `shopLastOpenedOn`
  - `gachaLastOpenedOn`
  - `problemLogs`

`registerProgress` は以下を保持:

- コイン（そろばんレジゲームと英語単語ゲームで共通）
- 購入済みアイテム
- 所持カード（カードIDを `purchasedItemIds` に保存）
- 所持シール枚数（`ownedStickerCounts`）
- シール帳配置（`stickerPlacements`）
- ゲーム内バッジ（`badgeIds`）
- 棚サイズ/スロット
- 現在開いている棚ID（`activeShelfId`）
- 棚ごとの配置データ（`shelfLayouts`）
- 解放済み級・種目ステージ
- 暗算種目のステージクリア状況（該当級が解放済みなら追加チャレンジとして選択可能）

`problemLogs` はレジゲームの問題単位ログを保持:

- 回答日
- 級・種目・ステージ
- 復習モードかどうか
- 正解/間違い結果
- 同じ問題での誤答入力回数

保持期間は直近180日。`state.ts` の正規化で古いログや不正値を除外する。

`state.ts` 内で正規化関数を通し、欠損や不正値を補正する設計。

## 8. UI層（ui/components）

再利用可能な軽量部品を格納。

- `Button`
- `Select`
- `NumberInput`
- `Toggle`

各 feature はこれらを組み合わせて画面を構築する。

## 9. スタイリング方針

- Tailwind utility をベースに実装。
- `src/styles/index.css` で:
  - フォント変数（`--sheet-font`, `--pop-font`）
  - 印刷時ルール
  - 正誤演出アニメーション（`flash-good`, `flash-bad`）

## 10. PWA/オフライン

- `src/main.tsx` で `/index.html` の更新監視（初回 + hashページ遷移時 + フォーカス復帰時）を行い、実行中エントリJSと差分があれば自動リロードする。
- `src/main.tsx` で `navigator.serviceWorker.register("/sw.js")` を実行。
- 更新版Service Workerがインストール済みになったら `SKIP_WAITING` を送信し、`controllerchange` で自動リロードして最新資産へ切り替える。
- `public/sw.js` ではアプリシェル（`/`, `/index.html`, manifest, icon）を事前キャッシュ。
- ナビゲーション要求はネットワーク優先、失敗時はキャッシュした `index.html` を返す。
- 同一オリジンの静的アセットはキャッシュ優先で再利用。

## 11. ビルド・実行

`package.json` scripts:

- `npm run dev` : 通常起動
- `npm run dev:lan` : LAN内のiPadなど実機から確認できるよう `0.0.0.0` で開発サーバーを起動
- `npm run dev:admin` : adminモード起動
- `npm run build` : 本番ビルド
- `npm run preview` : ビルド確認
- `npm run preview:lan` : LAN内の実機から確認できるよう `0.0.0.0` でビルド成果物をプレビュー
- `npm run typecheck` : 型検査

## 12. 変更時のガイドライン（AI向け）

1. 出題仕様変更は `domain/specs` と `domain/generator` を起点に行う。
2. 画面挙動変更は `features/*` に閉じる。
3. 保存仕様変更時は `state.ts` の正規化・後方互換を維持する。
4. ルート追加時は `App.tsx` の `Route` 型と hash パーサを同時更新する。
5. 変更後は最低 `npm run build` を実行して破壊的変更を検知する。

## 13. 関連ドキュメント

- `docs/soroban-game-mode-spec.md`
- `docs/soroban-practice-mode-spec.md`
- `docs/soroban-test-mode-spec.md`
- `docs/zenshugakuren-rules.md`
