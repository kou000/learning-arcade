...
- ステージ選択画面では、選択中の種目が有効なキャンペーン対象の場合のみ、左上にキャンペーン内容を表示する
  - 右上の所持コイン表示の下に、選択中の級のバッジ欄を常に表示する
  - バッジ欄はクリーム〜ゴールドのグラデーションと立体的な金色の枠で装飾し、各バッジに丸いくぼみ風の背景を表示する
  - バッジ欄には通常・暗算の各種目で獲得した最高ランクを種目名とともに表示し、級の切替に追従する。未獲得でも種目名を表示し、画像部分は同じ高さの空欄にする
  - バッジ欄は常に横1列の3枠とし、通常種目を選択中は「みとり・かけ・わり」、暗算種目を選択中は「みとりあんざん・かけあんざん・わりあんざん」を表示する。未獲得の種目の位置は詰めず、ラベルを残す
  - バッジ画像の読み込みに失敗した場合も、種目名を表示する
  - バッジ画像は64px四方の枠に収めて表示する（列幅が狭い場合は縮小）。獲得済みのバッジは種目名の下にランク名（ブロンズ・シルバー・ゴールド）を表示し、画像の代替テキストにも設定する
  - ランク名の文字色はバッジに合わせ、ブロンズは銅色、シルバーは銀色、ゴールドは金色とする。背景上で読みやすい濃さを保つ
  - バッジ欄とキャンペーンが同時に表示される場合は、キャンペーンを左上に配置する
- 問題数:
  - 見取り算:
    - ステージ1: 2問
    - ステージ2: 2問
...
- 役割:
  ...
  - 回答入力（テンキー上部にレジ表示風の大型ディスプレイを表示）
    - 入力値が表示幅を超える場合は、符号・小数点を含む文字幅に合わせてフォントサイズを自動縮小する
    - 桁数が減ったときや画面幅が変わったときは再調整し、表示欄の高さと右寄せを維持する
    - レシート・問題は縮めずに表示し、おたすけヒントはレシートの外、左パネル下部の独立した縦スクロール欄に表示する。ヒント欄は高さ64〜128pxの範囲で残りの高さに合わせ、入力欄・回答操作と右側のレジを押し出さない
    - レシートは数字の18px表示を維持し、行間と余白を詰めて問題全体を見渡せるようにする。極端に高さが足りない場合は左パネル全体もスクロールできる
    - 狭い画面の縦1列表示では問題・レジ全体を縦スクロールできる。高さが足りない横画面ではレジ内もスクロールでき、回答ボタンに到達できる
  - 正誤演出
  - 報酬付与
...
- みとり:
  ...
  - レシートの商品名と値段は内容幅の2列で近づけて表示し、値段は右寄せで桁をそろえる
  - 値段の文字サイズは18pxとし、商品名（14px）より大きく表示する
  - レシート枠は内容に合わせた幅とし、問題表示エリアの左右中央に配置する
- 見取暗算:
